...
func AddTasksToBuild(ctx context.Context, b *build.Build, project *Project, v *Version, taskNames []string,
	displayNames []string, tasksToActivate []string, generatedBy string, tasksInBuild []task.Task,
...
func AddNewBuilds(ctx context.Context, toActivate tasksAndVariantsToActivate, v *Version, p *Project,
	...
	for _, pair := range tasks.ExecTasks {
		...
		activateVariant := toActivate.activateVariant(pair.Variant)
		tasksToActivate := toActivate.tasks[pair.Variant]
		...
		taskStatuses := []TaskWithBatchTime{}
		if !activateVariant {
			...
			// we want the tasks that aren't already activated (i.e. tasks in taskNames that aren't in toActivate)
			batchTimeTasks, _ := utility.StringSliceSymmetricDifference(taskNames, toActivate.tasks[pair.Variant])
			// add tasks that require activation times
			for _, taskName := range batchTimeTasks {
				id, ok := taskNameToId[taskName]
				if !ok {
					continue
				}
...
func AddNewTasks(ctx context.Context, v *Version, p *Project, pairs TaskVariantPairs,
	...
	for _, b := range builds {
		...
		for _, taskname := range pairs.ExecTasks.TaskNames(b.BuildVariant) {
			...
			if !p.IsBatchtimeTaskForVariant(taskname, b.BuildVariant) {
				tasksToActivate = append(tasksToActivate, taskname)
			} else {
		...
		_, tasks, err := AddTasksToBuild(ctx, &b, p, v, tasksToAdd, displayTasksToAdd, tasksToActivate,
			generatedBy, tasksInBuild, syncAtEndOpts, distroAliases)
		if err != nil {
			return nil, err
		}
