...
// representation, and merges any new anchor definitions found into anchorRegistry.
func decodeWithAnchors(parseBytes []byte, unmarshalStrict bool, anchorRegistry *anchorEntries) (*ParserProject, error) {
	...
	if unmarshalStrict {
		strictProjectWithVariables := struct {
			...
			// EvgAnchors silences the "unknown field" error for the anchor preamble.
			EvgAnchors any `yaml:"_evg_anchors,omitempty"`
		}{}
		d := yaml.NewDecoder(bytes.NewReader(parseBytes))
		d.KnownFields(true)
		if err := d.Decode(&strictProjectWithVariables); err != nil {
			return nil, errors.Wrap(thirdparty.YAMLFormatError{Message: err.Error()}, "unmarshalling parser project from YAML")
		}
		...
		if err := node.Decode(&p); err != nil {
			...
			grip.Warning(context.Background(), message.Fields{
				"message": "yaml.v3 node decode failed, fell back to yaml.v2",
				"error":   err.Error(),
			})
		}
	}
