...
// anchorModuleIncludeOpts returns module include opts for cross-file anchor tests.
// Cross-file anchor support is always enabled, so this is equivalent to moduleIncludeOpts.
...
func TestAliasInMainReferencingIncludeAnchorShouldError(t *testing.T) {
	// The main file tries to use *from-include, but that anchor lives in
	// include.yml which has not been parsed yet when the main file is decoded.
	mainYAML := mainYAMLWithModuleIncludes(`
tasks:
- name: main-task
  commands:
